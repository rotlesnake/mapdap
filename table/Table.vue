<template>
  <section id="table">
    ...
    >
      ...
      <template v-slot:top v-if="showTop">
        ...
        <v-toolbar v-else flat color="toolbar">
          ...
          <v-divider inset vertical v-if="showOldFilter"></v-divider>
          <v-btn color="primary" class="ml-2" v-if="showOldFilter" @click="rowFilterDialog = true"> <v-icon>search</v-icon> Фильтр </v-btn>
          <v-btn
            color="red"
            class="ml-1"
            fab
            x-small
            v-if="tableFilter && showOldFilter"
            @click.stop="
              tableFilter = null;
              rowFilterValues = {};
            "
      ...
      <template #header="{ props, on }">
        <td v-for="head in props.headers" style="border-bottom: 1px solid #999; padding: 2px 5px">
          <input
            v-show="head.name"
            v-if="!['linkTable', 'select'].includes(head.type)"
            type="text"
            style="max-width: 150px; text-align: center; border: 1px solid #ccc; border-radius: 8px"
            @input="onInputHeaderFilter(head.type, head.name, $event.target.value)"
          />
        </td>
      </template>

      <template #header.data-table-select="{ props, on }">
          <v-checkbox dense hide-details @change="changeMultiselectAll" />
      </template>
      ...
      <template v-slot:item="{ item, select, isSelected, expand, isExpanded }">
        ...
        >
          ...
          <template v-for="columnItem in columnsFiltered">
            <td
              :class="[{ 'text-center': columnItem.align }, columnItem.class]"
              :key="columnItem.id"
        ...
        <template v-if="!$slots.expanded">
          ...
          <template v-for="(ex_table, ex_id) in info.childrenTables">
            <td :key="ex_id" :colspan="columnsFiltered.length" class="left">
              <table-editor
                :tableName="ex_table.table"
                :tableParent="[{ table: tableName, field: ex_table.field, value: item.id }]"
                :minHeight="200"
                :showTop="false"
              ></table-editor>
            </td>
      ...
      <template v-slot:body.append="{ headers }" v-if="appendRowData">
        <tr class="footer-row">
          <template v-for="head_item in columnsFiltered">
            <td class="footer-cell" :key="head_item.id" v-html="renderItem(head_item, appendRowData, true)"></td>
          </template>
        </tr>
      </template>
      ...
      <template v-slot:footer v-if="showFooter">
        ...
        <div class="table-footer pa-2" v-else>
          ...
          <v-tooltip top color="green">
            <template v-slot:activator="{ on }">
              <v-btn color="green" class="mx-2" v-if="userHasRoles(info.add)" fab small v-on="on" @click="openEditDialog('add')">
    ...
    <mdp-column-filter v-model="localColumns" :visible="columnFilterDialog"  @close="columnFilterDialogClose()" />
    <mdp-row-filter
      v-model="tableFilter"
      :info="info"
    ...
    <v-navigation-drawer v-model="filterDrawer" right app color="toolbar" class="pa-2" width="300" v-if="showTop">
      ...
      <template v-for="(item, i) in filterFields">
        <mdp-form-field :options="item" :key="i" :name="item.name" v-model="filterRow[item.name]" :row="filterRow" />
      </template>
...
export default {
  ...
  created() {
    this.remoteFilterRequest = common.debounce(this.remoteTableFilterRequest, 900);
    this.onInputHeaderFilterEnd = common.debounce(this.applyLocalTableFilter, 900);
  },
  mounted() {
    this.rowFilterValues = {};
    this.reloadTable();
  ...
  methods: {
    ...
    onInputHeaderFilter(fld_type, name, vals) {
      if (!this.localTableFilter) this.localTableFilter = [];
      
      name = name.replace("_text", "");
      this.localTableFilter = this.localTableFilter.filter((fld) => fld.field != name);
      ...
      vals.split("|").forEach((val)=>{
          ...
          let fltOper = "like";
          if ([">=","<=","in"].includes(val.substr(0,2))) { fltOper = val.substr(0,2); val = val.substr(2); }
          if (["=",">","<"].includes(val.substr(0,1))) { fltOper = val.substr(0,1); val = val.substr(1); }
          if (fld_type == "select") fltOper = "=";
          this.localTableFilter.push({ field: name, type: fltOper, value: val });

          //if (val == "null" || val === null) this.localTableFilter = this.localTableFilter.filter((fld) => fld.field != name);
      })

      this.onInputHeaderFilterEnd();
    },
    applyLocalTableFilter() {
        this.tableFilter = JSON.parse(JSON.stringify(this.localTableFilter));
    },
    ...
    convertColumns(columns, isTable) {
      ...
      return rez;
        ...
        columnFilterDialogClose() {
            this.columnFilterDialog = false;
            localStorage.setItem("table_settings_"+this.tableName, JSON.stringify(this.localColumns) );
        },
        localColumnsRestore() {
            let json = JSON.parse( localStorage.getItem("table_settings_"+this.tableName) );
            if (json) {
                json.forEach(e=>{
                    let c = this.localColumns.find(ce=>ce.name==e.name);
                    if (c) c.hidden = e.hidden;
                });
            }
        },
    ...
    remoteTableFilterRequest() {
      ...
      this.rows = [];
      this.$axios
        .post(this.filterUrl, { search: this.remoteTextFilter })
        .then((response) => {
          this.isLoading = false;
          response = response.data;
          this.rows = response.rows;
        ...
        if (Array.isArray(json)) {
          json.forEach((row) => {
            ...
            head.json.columns.forEach((col) => {
              ...
              if (row[col.name + "_text"]) {
                values += row[col.name + "_text"] + "|";
              } else {
                values += row[col.name] + "|";
              }
